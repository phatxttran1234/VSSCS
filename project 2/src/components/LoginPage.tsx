...
// Cute animated character component
const PasswordCharacter = ({ isPasswordFocused }: { isPasswordFocused: boolean }) => {
  return (
    <div className="flex justify-center mb-6">
      <div className="relative">
        {/* Character face */}
        <div className="w-20 h-20 bg-gradient-to-br from-orange-300 to-orange-400 rounded-full shadow-lg relative overflow-hidden">
          {/* Eyes */}
          <div className="absolute top-6 left-4 flex space-x-2">
            <div className={`w-3 h-3 bg-gray-800 rounded-full transition-all duration-500 ${
              isPasswordFocused ? 'scale-0 opacity-0' : 'scale-100 opacity-100'
            }`}></div>
            <div className={`w-3 h-3 bg-gray-800 rounded-full transition-all duration-500 ${
              isPasswordFocused ? 'scale-0 opacity-0' : 'scale-100 opacity-100'
            }`}></div>
          ...
          {/* Hands covering eyes */}
          <div className={`absolute top-4 left-2 flex space-x-1 transition-all duration-500 ${
            isPasswordFocused ? 'scale-100 opacity-100' : 'scale-0 opacity-0'
          }`}>
            <div className="w-4 h-4 bg-orange-200 rounded-full transform rotate-12"></div>
            <div className="w-4 h-4 bg-orange-200 rounded-full transform -rotate-12"></div>
          </div>
          ...
          <div className={`absolute bottom-6 left-1/2 transform -translate-x-1/2 transition-all duration-300 ${
            isPasswordFocused 
              ...
              : 'w-4 h-2 bg-gray-700 rounded-full'
          }`}></div>
          
          {/* Blush when covering eyes */}
          <div className={`absolute top-8 left-1 w-2 h-2 bg-red-300 rounded-full transition-all duration-500 ${
            isPasswordFocused ? 'opacity-60' : 'opacity-0'
          }`}></div>
          <div className={`absolute top-8 right-1 w-2 h-2 bg-red-300 rounded-full transition-all duration-500 ${
            isPasswordFocused ? 'opacity-60' : 'opacity-0'
          }`}></div>
        </div>
        
        {/* Character body */}
        <div className="w-16 h-12 bg-gradient-to-br from-orange-400 to-red-400 rounded-b-full mx-auto -mt-2 shadow-md"></div>
        
        {/* Volleyball */}
        <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-white rounded-full shadow-md border-2 border-orange-300">
          <div className="absolute inset-1 border border-orange-300 rounded-full"></div>
          <div className="absolute top-1/2 left-0 right-0 h-px bg-orange-300"></div>
          <div className="absolute left-1/2 top-0 bottom-0 w-px bg-orange-300"></div>
        </div>
        
        {/* Floating hearts when shy */}
        {isPasswordFocused && (
          <>
            <div className="absolute -top-2 -left-2 text-pink-400 animate-bounce">
              <Heart className="h-3 w-3 fill-current" />
            </div>
            <div className="absolute -top-1 -right-3 text-pink-400 animate-bounce delay-300">
      ...
      <div className="max-w-md w-full">
        ...
        {!showAdminForm ? (
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-purple-200">
            ...
            <div className="p-4">
              {/* Cute Character */}
              <PasswordCharacter isPasswordFocused={isPasswordFocused} />
